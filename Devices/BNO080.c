...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
#define CHIP_SELECT(XSPI)		ISpiChipSelect(XSPI, 0)
#define CHIP_DESELECT(XSPI)		ISpiChipSelect(XSPI, 1)

#define WAKE_HIGH(XGPO)				WriteGpio(XGPO, 1)
#define WAKE_LOW(XGPO)				WriteGpio(XGPO, 0)

#define RESET_HIGH(XGPO)			WriteGpio(XGPO, 1)
#define RESET_LOW(XGPO)				WriteGpio(XGPO, 0)
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////

extern ISpi* g_pxComm_Bno080;
extern GpioNode_t g_xGpo_Bno080_mode1;
extern GpioNode_t g_xGpo_Bno080_reset;
...
BNO080_Hw g_xBno080_hw;

void BNO080_Hw_Init(BNO080_Hw* self, ISpi* pxSpiPort, GpioNode_t* pxGpoReset, GpioNode_t* pxGpoWakeup, GpioNode_t* pxGpeInt){

	self->m_pxSpiPort = pxSpiPort;
	self->m_pxDataAvailable = pxGpeInt;
	self->m_pxReset = pxGpoReset;
	self->wakeup = pxGpoWakeup;
}

int BNO080_Initialization(BNO080_Hw* pxHw)
{
	BNO080_GPIO_SPI_Initialization();
	
	printf("Checking BNO080...");
	
	CHIP_DESELECT(pxHw->m_pxSpiPort);
	
	//Configure the BNO080 for SPI communication
	WAKE_HIGH(pxHw->wakeup);	//Before boot up the PS0/WAK pin must be high to enter SPI mode
	RESET_LOW(pxHw->m_pxReset);	//Reset BNO080
	HAL_Delay(200);	//Min length not specified in datasheet?
	RESET_HIGH(pxHw->m_pxReset);	//Bring out of reset
	
	BNO080_waitForSPI(pxHw); //Wait until INT pin goes low.
	
	//At system startup, the hub must send its full advertisement message (see 5.2 and 5.3) to the
	//host. It must not send any other data until this step is complete.
	//When BNO080 first boots it broadcasts big startup packet
	//Read it and dump it
	BNO080_waitForSPI(pxHw); //Wait for assertion of INT before reading advert message.
	BNO080_receivePacket(pxHw);
	
	//The BNO080 will then transmit an unsolicited Initialize Response (see 6.4.5.2)
	//Read it and dump it
	BNO080_waitForSPI(pxHw);  //Wait for assertion of INT before reading Init response
	BNO080_receivePacket(pxHw);
	...
	BNO080_sendPacket(pxHw, CHANNEL_CONTROL, 2);
	
	//Now we wait for response
	BNO080_waitForSPI(pxHw);
	if (BNO080_receivePacket(pxHw) == 1)
	{
...
unsigned char SPI2_SendByte(BNO080_Hw* pxHw, unsigned char data)
{
	// while(LL_SPI_IsActiveFlag_TXE(BNO080_SPI_CHANNEL)==RESET);
	// LL_SPI_TransmitData8(BNO080_SPI_CHANNEL, data);

	ISpiWriteByte(pxHw->m_pxSpiPort, data);
	...
	u8 rtn = 0; 
	ISpiReadByte(pxHw->m_pxSpiPort, &rtn);
...
int BNO080_dataAvailable(BNO080_Hw* pxHw)
{
	...
	if(ReadGpio(pxHw->m_pxDataAvailable) != 0){
		return (0);
	}
...
float BNO080_getRange(BNO080_Hw* pxHw, uint16_t recordID)
{
	//The resolution Q value are 'the same as those used in the sensor's input report'
	//This should be Q1.
	int16_t Q = BNO080_getQ1(pxHw, recordID);
	...
	uint32_t value = BNO080_readFRSword(pxHw, recordID, 1); //Get word 1
...
uint32_t BNO080_readFRSword(BNO080_Hw* pxHw, uint16_t recordID, uint8_t wordNumber)
{
	if (BNO080_readFRSdata(pxHw, recordID, wordNumber, 1) == 1) //Get word number, just one word in length from FRS
...
void BNO080_frsReadRequest(BNO080_Hw* pxHw, uint16_t recordID, uint16_t readOffset, uint16_t blockSize)
{
	...
	BNO080_sendPacket(pxHw, CHANNEL_CONTROL, 8);
}
...
int BNO080_readFRSdata(BNO080_Hw* pxHw, uint16_t recordID, uint8_t startLocation, uint8_t wordsToRead)
{
	uint8_t spot = 0;

	//First we send a Flash Record System (FRS) request
	BNO080_frsReadRequest(pxHw, recordID, startLocation, wordsToRead); //From startLocation of record, read a # of words
	...
	while (1)
	{
		...
		{
			uint8_t counter = 0;
			while (BNO080_receivePacket(pxHw) == 0)
			{
				if (counter++ > 100)
					return (0); //Give up
				HAL_Delay(1);
...
void BNO080_softReset(BNO080_Hw* pxHw)
{
	shtpData[0] = 1; //Reset

	//Attempt to start communication with sensor
	BNO080_sendPacket(pxHw, CHANNEL_EXECUTABLE, 1); //Transmit packet on channel 1, 1 byte

	//Read all incoming data and flush it
	HAL_Delay(50);
	while (BNO080_receivePacket(pxHw) == 1);
	HAL_Delay(50);
	while (BNO080_receivePacket(pxHw) == 1);
}
...
uint8_t BNO080_resetReason(BNO080_Hw* pxHw)
{
	shtpData[0] = SHTP_REPORT_PRODUCT_ID_REQUEST; //Request the product ID and reset info
	shtpData[1] = 0;							  //Reserved
...
void BNO080_enableRotationVector(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_ROTATION_VECTOR, timeBetweenReports, 0);
}

//Sends the packet to enable the rotation vector
void BNO080_enableGameRotationVector(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_GAME_ROTATION_VECTOR, timeBetweenReports, 0);
}

//Sends the packet to enable the accelerometer
void BNO080_enableAccelerometer(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_ACCELEROMETER, timeBetweenReports, 0);
}

//Sends the packet to enable the accelerometer
void BNO080_enableLinearAccelerometer(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_LINEAR_ACCELERATION, timeBetweenReports, 0);
}

//Sends the packet to enable the gyro
void BNO080_enableGyro(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_GYROSCOPE, timeBetweenReports, 0);
}

//Sends the packet to enable the magnetometer
void BNO080_enableMagnetometer(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_MAGNETIC_FIELD, timeBetweenReports, 0);
}

//Sends the packet to enable the step counter
void BNO080_enableStepCounter(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_STEP_COUNTER, timeBetweenReports, 0);
}

//Sends the packet to enable the Stability Classifier
void BNO080_enableStabilityClassifier(BNO080_Hw* pxHw, uint16_t timeBetweenReports)
{
	BNO080_setFeatureCommand(pxHw, SENSOR_REPORTID_STABILITY_CLASSIFIER, timeBetweenReports, 0);
}

//Sends the commands to begin calibration of the accelerometer
void BNO080_calibrateAccelerometer(BNO080_Hw* pxHw)
{
	BNO080_sendCalibrateCommand(pxHw, CALIBRATE_ACCEL);
}

//Sends the commands to begin calibration of the gyro
void BNO080_calibrateGyro(BNO080_Hw* pxHw)
{
	BNO080_sendCalibrateCommand(pxHw, CALIBRATE_GYRO);
}

//Sends the commands to begin calibration of the magnetometer
void BNO080_calibrateMagnetometer(BNO080_Hw* pxHw)
{
	BNO080_sendCalibrateCommand(pxHw, CALIBRATE_MAG);
}

//Sends the commands to begin calibration of the planar accelerometer
void BNO080_calibratePlanarAccelerometer(BNO080_Hw* pxHw)
{
	BNO080_sendCalibrateCommand(pxHw, CALIBRATE_PLANAR_ACCEL);
}

//See 2.2 of the Calibration Procedure document 1000-4044
void BNO080_calibrateAll(BNO080_Hw* pxHw)
{
	BNO080_sendCalibrateCommand(pxHw, CALIBRATE_ACCEL_GYRO_MAG);
}

void BNO080_endCalibration(BNO080_Hw* pxHw)
{
	BNO080_sendCalibrateCommand(pxHw, CALIBRATE_STOP); //Disables all calibrations
}
...
void BNO080_setFeatureCommand(BNO080_Hw* pxHw, uint8_t reportID, uint32_t microsBetweenReports, uint32_t specificConfig)
{
	...
	BNO080_sendPacket(pxHw, CHANNEL_CONTROL, 17);
}

//Tell the sensor to do a command
//See 6.3.8 page 41, Command request
//The caller is expected to set P0 through P8 prior to calling
void BNO080_sendCommand(BNO080_Hw* pxHw, uint8_t command)
{
	...
	BNO080_sendPacket(pxHw, CHANNEL_CONTROL, 12);
}

//This tells the BNO080 to begin calibrating
//See page 50 of reference manual and the 1000-4044 calibration doc
void BNO080_sendCalibrateCommand(BNO080_Hw* pxHw, uint8_t thingToCalibrate)
{
	...
	BNO080_sendCommand(pxHw, COMMAND_ME_CALIBRATE);
}

//Request ME Calibration Status from BNO080
//See page 51 of reference manual
void BNO080_requestCalibrationStatus(BNO080_Hw* pxHw)
{
...
void BNO080_saveCalibration(BNO080_Hw* pxHw)
{
	/*shtpData[3] = 0; //P0 - Reserved
	shtpData[4] = 0; //P1 - Reserved
	shtpData[5] = 0; //P2 - Reserved
	...
	//Using this shtpData packet, send a command
	BNO080_sendCommand(pxHw, COMMAND_DCD); //Save DCD command
}

//Blocking wait for BNO080 to assert (pull low) the INT pin
//indicating it's ready for comm. Can take more than 104ms
//after a hardware reset
int BNO080_waitForSPI(BNO080_Hw* pxHw)
{
...
int BNO080_receivePacket(BNO080_Hw* pxHw)
{
	uint8_t incoming;
	...
	CHIP_SELECT(pxHw->m_pxSpiPort);

	//Get the first four bytes, aka the packet header
	uint8_t packetLSB = SPI2_SendByte(pxHw, 0);
	uint8_t packetMSB = SPI2_SendByte(pxHw, 0);
	uint8_t channelNumber = SPI2_SendByte(pxHw, 0);
	uint8_t sequenceNumber = SPI2_SendByte(pxHw, 0); //Not sure if we need to store this or not
	...
	{
		incoming = SPI2_SendByte(pxHw, 0xFF);
		//printf("%d ", incoming);
	...
	CHIP_DESELECT(pxHw->m_pxSpiPort); //Release BNO080
	return (1); //We're done!
}
...
int BNO080_sendPacket(BNO080_Hw* pxHw, uint8_t channelNumber, uint8_t dataLength)
{
	uint8_t packetLength = dataLength + 4; //Add four bytes for the header

	//Wait for BNO080 to indicate it is available for communication
	if (BNO080_waitForSPI(pxHw) == 0)
		return (0); //Data is not available
	...
	SPI2_SendByte(pxHw, packetLength & 0xFF);			//Packet length LSB
	SPI2_SendByte(pxHw, packetLength >> 8);				//Packet length MSB
	SPI2_SendByte(pxHw, channelNumber);					//Channel number
	SPI2_SendByte(pxHw, sequenceNumber[channelNumber]++); 	//Send the sequence number, increments with each packet sent, different counter for each channel

	//Send the user's data packet
	for (uint8_t i = 0; i < dataLength; i++)
	{
		SPI2_SendByte(pxHw, shtpData[i]);
	}
